...
- **Здоров'я** — журнал подій здоров'я в місячному календарі: заміри тиску/пульсу
  (структуровані поля з валідацією), головний біль і мігрень із тяжкістю 1–5,
  ліки, нотатки та власні типи подій. Нижче — графіки САТ/ДАТ і пульсу за обраний
  період (пресети тиждень/місяць/3 місяці) на часовій осі з пунктирними межами
  норми 135/85. Будь-які типи подій можна **накласти на графік** і дивитись
  кореляції: тяжкість — точками/стовпчиками/лінією на правій осі 1–5, події без
  числа — вертикальними маркерами чи смугою на день. Колір і стиль кожного типу
  налаштовуються (⚙ Вигляд) і зберігаються на бекенді.
...
- **Залежності на фронтенді — лише за ділом.** База — Vue, Router, Pinia, Vite;
  графік ваги намальований вручну через SVG. Для журналу здоров'я свідомо взяті
  спеціалізовані бібліотеки: FullCalendar (місячна сітка подій) і Chart.js
  з annotation-плагіном (медичні графіки з зонами норми) — там, де самопис
  коштував би дорожче за виграш.
